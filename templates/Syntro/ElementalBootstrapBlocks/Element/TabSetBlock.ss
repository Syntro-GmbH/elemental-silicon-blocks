<div class="container">
  ...
  <ul class="nav nav-tabs-alt" role="tablist">
    <% loop Tabs %>
      ...
      <a href="#" class="nav-link text-nowrap<% if First %> active<% end_if %>" id="Tabset-{$Up.ID}-{$ID}-tab" data-bs-toggle="tab" data-bs-target="#Tabset-{$Up.ID}-{$ID}" type="button" role="tab" aria-controls="Tabset-{$Up.ID}-{$ID}" aria-selected="true">
          <% if Icon %>
