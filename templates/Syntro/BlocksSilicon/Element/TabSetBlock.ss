<div class="container">
  ...
  <ul class="nav nav-tabs flex-nowrap justify-content-lg-center overflow-auto pb-2 mb-3 mb-lg-4" role="tablist">
    <% loop Tabs %>
      ...
      <button class="nav-link text-nowrap<% if First %> active<% end_if %>" id="Tabset-{$Up.ID}-{$ID}-tab" data-bs-toggle="tab" data-bs-target="#Tabset-{$Up.ID}-{$ID}" type="button" role="tab" aria-controls="Tabset-{$Up.ID}-{$ID}" aria-selected="true">
          <% if Icon %>
